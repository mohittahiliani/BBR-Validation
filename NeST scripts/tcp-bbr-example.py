...
# Disable offloads
with sender:
	for offload in ['gso', 'gro', 'tso']:
		subprocess.Popen(f'ethtool --offload {sender_router1.id} {offload} off'.split())
       
with router1:
        for offload in ['gso', 'gro', 'tso']:
    	        subprocess.Popen(f'ethtool --offload {router1_router2.id} {offload} off'.split())

with router2:
  	for offload in ['gso', 'gro', 'tso']:
  		subprocess.Popen(f'ethtool --offload {router2_router1.id} {offload} off'.split())	

with receiver:
  	for offload in ['gso', 'gro', 'tso',]:
  		subprocess.Popen(f'ethtool --offload {receiver_router2.id} {offload} off'.split())
